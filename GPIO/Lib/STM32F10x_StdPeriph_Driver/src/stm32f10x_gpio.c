...
void GPIO_Init(GPIO_TypeDef* GPIOx, GPIO_InitTypeDef* GPIO_InitStruct)
{
  ...
  //DBG("Entering GPIO_Init()\n\r");
  
/*---------------------------- GPIO Mode Configuration -----------------------*/
  /* Get the current mode the user configured */
  currentmode = ((uint32_t)GPIO_InitStruct->GPIO_Mode) & ((uint32_t)0x0F);
  /* Current mode # reserved mode (0x00) */
...
/*---------------------------- GPIO CRL Configuration ------------------------*/
  ...
  if (((uint32_t)GPIO_InitStruct->GPIO_Pin & ((uint32_t)0x00FF)) != 0x00)
  {
    ...
    {
      /* Shift bit from 0 to 8 */
      pos = ((uint32_t)0x01) << pinpos;
      currentpin = (GPIO_InitStruct->GPIO_Pin) & pos;
      /* Detect bit set */
      if (currentpin == pos)
      {
    	/* Bit 5 is set => pos = 5 * 4 = 20 */
        pos = pinpos << 2;
...
void GPIO_SetBits(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin)
{
  ...
  //DBG("Default Value of BSRR \n\r");
  //Display_Binary(GPIOx->BSRR);
  GPIOx->BSRR = GPIO_Pin;
  //DBG("Current Value of BSRR \n\r");
  //Display_Binary(GPIOx->BSRR);
}
...
void GPIO_ResetBits(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin)
{
  ...
  assert_param(IS_GPIO_ALL_PERIPH(GPIOx));
  assert_param(IS_GPIO_PIN(GPIO_Pin));
  //DBG("Default Value of ODR \n\r");
  //Display_Binary(GPIOx->ODR);
  GPIOx->BRR = GPIO_Pin;
  //DBG("Current Value of ODR \n\r");
  //Display_Binary(GPIOx->ODR);
}
